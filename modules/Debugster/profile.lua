...
local function _hooker(event)
  ...
  if not _labeled[f] then
    ...
    if not _defined[f] then
      print (format("%s:%s", info.short_src, info.linedefined))
      _defined[f] = format("%s:%s", info.short_src, info.linedefined)
      _ncalls[f] = 0
      _rcount[f] = 0
      _telapsed[f] = 0
      _min[f]      = 9999999999999
      _max[f]      = -1
    end
  end
  ...
  elseif event == "return" then
    ...
    if rc then
      ...
      if rc == 1 and _tcalled[f] then
        ...
        _min[f] = dt< _min[f] and dt or _min[f]
        _max[f] = dt >_max[f] and dt or _max[f]
...
local function _iterator()
  ...
  _i = _i - 1
  local d = _defined[f]
  return _labeled[f] or d, _ncalls[f] or 0, _telapsed[f] or 0, d,_min[f],_max[f]
end
